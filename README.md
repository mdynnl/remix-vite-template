...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create beautech-3ab6
  fly apps create beautech-3ab6-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app beautech-3ab6
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app beautech-3ab6-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'beautech-3ab6-staging' does not match app name in config file 'beautech-3ab6'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name beautech-3ab6-db
  fly postgres attach --app beautech-3ab6 beautech-3ab6-db

  fly postgres create --name beautech-3ab6-staging-db
  fly postgres attach --app beautech-3ab6-staging beautech-3ab6-staging-db
  ```
